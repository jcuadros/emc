...
Si se desea o dispone de un archivo delimitado por tabulaciones, el carácter tabular se indica como `\t`.
...
Entre las operacions habituales que haremos sobre una tabla de datos, hay

- renombrar filas o columnas,
- añadir columnas o filas,
- segmentar,
- eliminar filas o columnas,
- crear tablas de datos de resumen,
- unir tablas, y
- cambiar el formato de la tabla (largo a ancho, y viceversa)

----

Partimos de una taba de datos sintética...
...
## Añadir columnas
dfTest2 <- cbind(dfTest1, rnorm(5)) # añadir un vector al data frame
dfTest2
dfTest2$var5 <- 5:1 # assignando valores a una nueva variable
dfTest2
...
## ... por condiciones o valores lógicos
...
humedad <- read.table("humedad.txt", sep="\t", dec=",", header=T)
colnames(humedad) <- c("muestra", "repeticion", "valor")

## Creación de una tabla de resumen a partir de datos en formato largo/tidy
humedad_muestras <- group_by(humedad, muestra) %>%
  summarise(xbar = mean(valor), r = diff(range(valor)), s = sd(valor))

## Conversión de formato largo a formato ancho
humedad_W1 <- as.data.frame(matrix(humedad$valor, byrow= T, ncol = 5))
colnames(humedad_W1) <- paste("rep", 1:5, sep="")

## Creación de resúmenes a partir de datos en formato ancho
humedad_W1$xbar <- apply(humedad_W1,1,mean)
humedad_W1$r <- apply(humedad_W1[,1:5],1,function(x) diff(range(x)))
humedad_W1$s <- apply(humedad_W1[,1:5],1,sd)

## Conversión de formato ancho a formato largo
humedad_L1 <- data.frame(muestra = rep(1:32, 5),
        repeticion = sort(rep(1:5, 32)))
humedad_L1$valor <- c(humedad_W1[,1],humedad_W1[,2],
                      humedad_W1[,3],humedad_W1[,4],
                      humedad_W1[,5])
humedad_L1 <- humedad_L1[order(humedad_L1$muestra),]

```{r, echo = TRUE}
# Estas conversiones también pueden hacerse a partir de las funciones
# spread y gather del paquete tidyr (incluido en tidyverse)
if(!require("tidyverse")) {
  install.packages("tidyverse")
  library("tidyverse")
}
...
humedad_W2 <- humedad %>% spread(repeticion,valor)

humedad_L2 <- humedad_W2 %>% gather("repeticion","valor",-muestra)
humedad_L2$repeticion <- as.integer(humedad_L2$repeticion)
humedad_L2 <- humedad_L2[order(humedad_L2$muestra),]
...
## Eliminar todas del espacio de trabajo
rm(list=ls())
